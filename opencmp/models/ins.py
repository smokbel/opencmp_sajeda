...
class INS(Model):
    ...
    def _post_init(self) -> None:
        ...
        self.save_freq = self.config.get_list(['VISUALIZATION', 'save_frequency'], str, quiet=True)
    ...
    def solve_single_step(self, a_lst: List[BilinearForm], L_lst: List[LinearForm],
                         precond_lst: List[Preconditioner], gfu: GridFunction, saver: SolutionFileSaver = None, time_step: int = 0) -> None:
        if self.linearize == 'Oseen':
            ...
            while not done_iterating:
                ...
                if saver:
                    print("Saving iteration ", num_iteration, self.save_freq[0])
                    if num_iteration % int(self.save_freq[0]) == 0:
                        saver.save(gfu, num_iteration)
